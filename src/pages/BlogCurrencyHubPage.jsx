/**
 * src/pages/BlogCurrencyHubPage.jsx
 * 
 * Purpose: Public currency taxonomy page listing posts related to a specific currency.
...
export default function BlogCurrencyHubPage() {
    ...
    // Fetch posts with this currency tag
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                ...
                // Filter posts that have this currency tag
                const filteredPosts = allPosts.filter(post =>
                    post.currencyTags?.includes(currency)
                );
                setPosts(filteredPosts);
            } catch (err) {
                console.error('Failed to fetch currency posts:', err);
            } finally {
                setLoading(false);
            }
        };
        ...
        fetchPosts();
    }, [currency, currentLang, isValidCurrency, t]);

    // SEO
    const langPrefix = currentLang !== 'en' ? `/${currentLang}` : '';
    const pageTitle = currencyLabel
        ? t('blog:currencyHub.title', { currency: currencyLabel })
        : t('blog:currencyHub.notFound', 'Currency Not Found');
    const pageDescription = currencyLabel
        ? t('blog:currencyHub.description', { currency: currencyLabel })
        : '';
    const canonicalUrl = `${SITE_URL}${langPrefix}/blog/currency/${currency?.toLowerCase()}`;

    // Structured data for CollectionPage
    const structuredData = isValidCurrency && posts.length > 0 ? {
    ...
    // 404 for invalid currency
    if (!loading && !isValidCurrency) {
        return (
                    ...
                    <>
                        ...
                        <Box sx={{ mb: 5 }}>
                            ...
                            <Typography variant="body1" color="text.secondary" sx={{ maxWidth: 800 }}>
                                {t('blog:currencyHub.intro', { currency: currencyLabel || currency, count: posts.length })}
                            </Typography>
                        </Box>
                        ...
                        {posts.length > 0 ? (
                            ...
                            <Alert severity="info">
                                {t('blog:currencyHub.noPosts', 'No posts found for this currency yet.')}
                            </Alert>
                        )}
                    </>
                )}
