/**
 * src/pages/BlogEventCurrencyHubPage.jsx
 * 
 * Purpose: Public combined event+currency taxonomy page listing posts related to both
 * a specific economic event AND a specific currency.
 * BEP: SEO-first with structured data, multi-language, theme-aware.
 * 
 * Routes: /blog/event/{eventKey}/{currency}, /{lang}/blog/event/{eventKey}/{currency}
 * Example: /blog/event/cpi/usd, /es/blog/event/nfp/eur
 * 
 * Changelog:
 ...
 * v1.0.0 - 2026-02-04 - Initial implementation (Phase 5.B Blog - Combined taxonomy)
 */
...
export default function BlogEventCurrencyHubPage() {
    const { eventKey, currency: currencyParam } = useParams();
    ...
    // Validate event key and currency
    const eventLabel = BLOG_ECONOMIC_EVENTS[eventKey];
    const isValidEvent = Boolean(eventLabel);
    const currencyLabel = BLOG_CURRENCY_LABELS[currency];
    const isValidCurrency = BLOG_CURRENCIES.includes(currency);
    const isValid = isValidEvent && isValidCurrency;

    // Fetch posts with BOTH this event tag AND currency tag
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                ...
                // Filter posts that have BOTH this event tag AND currency tag
                const filteredPosts = allPosts.filter(post =>
                    post.eventTags?.includes(eventKey) && post.currencyTags?.includes(currency)
                );
                setPosts(filteredPosts);
            } catch (err) {
                console.error('Failed to fetch event+currency posts:', err);
            } finally {
                setLoading(false);
            }
        };
        ...
        fetchPosts();
    }, [eventKey, currency, currentLang, isValid]);

    // SEO
    const langPrefix = currentLang !== 'en' ? `/${currentLang}` : '';
    const pageTitle = isValid
        ? t('blog:eventCurrencyHub.title', { event: eventLabel, currency: currencyLabel })
        : t('blog:eventCurrencyHub.notFound', 'Page Not Found');
    const pageDescription = isValid
        ? t('blog:eventCurrencyHub.description', { event: eventLabel, currency: currencyLabel })
        : '';
    const canonicalUrl = `${SITE_URL}${langPrefix}/blog/event/${eventKey}/${currency?.toLowerCase()}`;

    // Structured data for CollectionPage
    const structuredData = isValid && posts.length > 0 ? {
    ...
    // 404 for invalid event or currency
    if (!loading && !isValid) {
        const errorMessage = !isValidEvent
            ? t('blog:eventHub.notFound', 'Event not found')
            : t('blog:currencyHub.notFound', 'Currency not found');
        ...
        <PublicLayout navItems={navItems} onOpenAuth={handleOpenAuth} onOpenSettings={handleOpenSettings}>
            ...
            <Container maxWidth="lg" sx={{ py: { xs: 3, md: 6 } }}>
                ...
                <Breadcrumbs sx={{ mb: 3 }}>
                    ...
                    <Link component={RouterLink} to={`/blog/event/${eventKey}`} color="inherit" underline="hover">
                        {eventLabel}
                    </Link>
                    <Typography color="text.primary">{currencyLabel}</Typography>
                </Breadcrumbs>
                    ...
                    <>
                        ...
                        <Box sx={{ mb: 5 }}>
                            <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
                                ...
                                <AttachMoneyIcon color="secondary" sx={{ fontSize: 32, ml: -1 }} />
                                <Typography variant="h1" component="h1" sx={{ fontSize: { xs: '1.5rem', md: '2.25rem' }, fontWeight: 700 }}>
                                    {eventLabel} + {currencyLabel}
                                </Typography>
                            </Stack>
                            <Typography variant="body1" color="text.secondary" sx={{ maxWidth: 800 }}>
                                {t('blog:eventCurrencyHub.intro', { event: eventLabel, currency: currencyLabel, count: posts.length })}
                            </Typography>
                            ...
                            <Stack direction="row" spacing={1} sx={{ mt: 2 }}>
                                ...
                                <Chip
                                    ...
                                    label={currencyLabel}
                                    component={RouterLink}
                                    to={`/blog/currency/${currency?.toLowerCase()}`}
                                    clickable
                                    variant="outlined"
                                    color="secondary"
                                />
                        ...
                        {posts.length > 0 ? (
                            ...
                            <Alert severity="info">
                                {t('blog:eventCurrencyHub.noPosts', 'No posts found for this event and currency combination yet.')}
                            </Alert>
                        )}
                    </>
                )}
