...
export default function Calendar2Page() {
    ...
    const {
        ...
        favoritesLoading,
        applyFilters,
    } = useCalendarData({ defaultPreset: 'today' });

    // ─── Initialize 'Today' filter on mount for all users (guests & auth) ───
    useEffect(() => {
        // Apply 'today' preset on initial mount to ensure events table loads
        applyFilters({ dateRange: 'today' });
    ...
    const handleSaveCustomEvent = useCallback(() => {
        ...
        // If authenticated, dialog will be handled by CustomEventDialog's save logic
        setCustomDialogOpen(false);
    }, [isAuthenticated]);
        ...
        <Box>
            ...
            <Suspense fallback={null}>
                <ClockEventsFilters
                    ...
                    timezone={timezone}
                    disabled={isLoadingClockEvents || loading}
                    sx={{ mb: 2.5 }}
                />
            </Suspense>
    ...
    return (
        <>
            ...
            {/* Jump to Now FAB */}
            <Zoom in={!loading && events.length > 0 && (nowEventIds.size > 0 || nextEventIds.size > 0) && !targetRowVisible}>
